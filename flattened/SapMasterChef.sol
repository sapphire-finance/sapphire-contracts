...
// File: contracts\libs\ISapReferral.sol



pragma solidity 0.6.12;

interface ISapReferral {
    /**
     * @dev Record referral.
     */
...
// File: contracts\SapToken.sol

pragma solidity 0.6.12;


// SapToken with Governance.
contract SapToken is BEP20('Sapphire', 'SAP') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "SAP::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SAP::delegateBySig: invalid nonce");
        require(now <= expiry, "SAP::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "SAP::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SAPs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "SAP::_writeCheckpoint: block number exceeds 32 bits");
...
// File: contracts\SapSyrupBar.sol

pragma solidity 0.6.12;



// SapSyrupBar with Governance.
contract SapSyrupBar is BEP20('SyrupBar Token', 'SYRUP') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    function safeSapTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 sapBal = sap.balanceOf(address(this));
        if (_amount > sapBal) {
            sap.transfer(_to, sapBal);
        } else {
            sap.transfer(_to, _amount);
        }
    }
...
// File: contracts\SapMasterChef.sol
