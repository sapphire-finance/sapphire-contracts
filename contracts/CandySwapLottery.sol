...
// File: contracts/interfaces/ICandySwapLottery.sol

pragma solidity ^0.8.4;

interface ICandySwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in CANDY per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in CANDY token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInCandy: price of a ticket in CANDY
...
// File: contracts/CandySwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title CandySwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract CandySwapLottery is ReentrancyGuard, ICandySwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInCandy = 50 ether;
    uint256 public minPriceTicketInCandy = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInCandy;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] candyPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInCandy;
        uint32 finalNumber;
    }
     ...
     * @param _candyTokenAddress: address of the CANDY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _candyTokenAddress, address _randomGeneratorAddress) {
        candyToken = IERC20(_candyTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of CANDY to this contract
        uint256 amountCandyToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInCandy,
            _ticketNumbers.length
        );

        // Transfer candy tokens to this contract
        candyToken.safeTransferFrom(address(msg.sender), address(this), amountCandyToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCandy += amountCandyToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInCandyToTransfer
        uint256 rewardInCandyToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCandyToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        candyToken.safeTransfer(msg.sender, rewardInCandyToTransfer);

        emit TicketsClaim(msg.sender, rewardInCandyToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCandy) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in CANDY for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].candyPerBracket[j] =
                ...
                // A. No CANDY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].candyPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInCandy - amountToShareToWinners);

        // Transfer CANDY to treasury address
        candyToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        candyToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInCandy += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInCandy >= minPriceTicketInCandy) && (_priceTicketInCandy <= maxPriceTicketInCandy),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInCandy: _priceTicketInCandy,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            candyPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInCandy: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(candyToken), "Cannot be CANDY token");
    ...
    /**
     * @notice Set CANDY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInCandy: minimum price of a ticket in CANDY
     * @param _maxPriceTicketInCandy: maximum price of a ticket in CANDY
     */
    function setMinAndMaxTicketPriceInCandy(uint256 _minPriceTicketInCandy, uint256 _maxPriceTicketInCandy)
        external
        onlyOwner
    {
        require(_minPriceTicketInCandy <= _maxPriceTicketInCandy, "minPrice must be < maxPrice");

        minPriceTicketInCandy = _minPriceTicketInCandy;
        maxPriceTicketInCandy = _maxPriceTicketInCandy;
    }
     ...
     * @param _priceTicket price of a ticket (in CANDY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].candyPerBracket[_bracket];
        } else {
            return 0;
        }
    }
